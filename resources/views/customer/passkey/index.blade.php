{{--
    ...
    ROUTES USED:
      ...
      GET  /customer/passkeys/register/options — Challenge holen (route('customer.passkeys.options'))
      POST /customer/passkeys/register         — Passkey speichern (route('customer.passkeys.register'))
      PATCH /customer/passkeys/{id}/rename     — Umbenennen (route('customer.passkeys.rename'))
      DELETE /customer/passkeys/{id}           — Löschen (route('customer.passkeys.destroy'))
      GET  /customer/dashboard                 — Zurück (route('customer.dashboard'))
--}}
<!DOCTYPE html>
<html lang="de">
<head>
...
<body class="min-h-screen bg-gray-50 text-gray-900 antialiased"
    ...
    <script>
        ...
        async function saveRename(id, name) {
            const res = await fetch(`/customer/passkeys/${id}/rename`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
