{{--
    FILE:    resources/views/faq/index_mand.blade.php
    VERSION: 1.0.0
    DATE:    2026-06-20

    DESCRIPTION:
      FAQ & Infos — Übersichtsseite für Galerist:innen (mand). Liste der
      vorhandenen .md-Dateien aus storage/app/private/faq/mand/, vertikal
      ...
      Standalone (kein Dashboard-Nav), Stil wie policy/mand_update.blade.php.
    ...
    ROUTES USED:
      GET mandant.faq.show/{slug} — JSON {title, html} für das Modal (fetch)
--}}
<!DOCTYPE html>
<html lang="de">
<head>
...
<body class="min-h-screen bg-gray-50 text-gray-900 antialiased"
    ...
    <div class="min-h-screen flex items-center justify-center px-4 py-12">
        <div class="w-full max-w-lg bg-white rounded-xl border border-gray-200
            ...
            <button type="button"
                    @click="window.location='{{ route('mandant.dashboard') }}'"
                    ...
                    @foreach($items as $item)
                        <button type="button"
                                @click="
                                    ...
                                    title = @js($item['label']);
                                    html = '';
                                    fetch('{{ route('mandant.faq.show', $item['slug']) }}')
                                        .then(r => r.json())
