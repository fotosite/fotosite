...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] flex p-6 lg:p-8 items-center lg:justify-center min-h-screen flex-col"
          x-data="{ showModal: @json(session('open_login_modal') === 'cust' || $errors->any()) }">
        ...
        <div x-show="showModal"
            ...
            <div class="relative w-full max-w-sm bg-white rounded-xl shadow-2xl px-8 py-8 overflow-y-auto max-h-[90vh]"
                 x-data="custLogin">
                ...
                {{-- Kopfzeile --}}
                <div class="mb-6">
                    <p class="text-[11px] font-mono tracking-widest uppercase text-gray-400 mb-1">Fotogalerie</p>
                ...
                <div class="flex border-b border-gray-200 mb-6">
                    <button type="button"
                            ...
                            class="flex-1 pb-3 text-sm font-medium text-center focus:outline-none transition-colors">
                        Mitglied
                    </button>
                    <button type="button"
                            @click="tab = 'anon'"
                ...
                <div x-show="tab === 'reg'">
                    ...
                    <button type="button"
                            ...
                            class="w-full flex items-center justify-center gap-2 rounded-lg border-2 border-indigo-600 px-4 py-3 md:py-2 text-sm font-semibold text-indigo-600 hover:bg-indigo-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                    ...
                    <form method="POST" action="{{ route('customer.login.handle') }}" autocomplete="off">
                        @csrf
                        <div class="space-y-4">
                            <div>
                                <label for="modal_cust_email"
                                       class="block text-sm font-medium text-gray-700 mb-1">E-Mail-Adresse</label>
                                <input id="modal_cust_email" name="cust_email" type="email" required
                                       value="{{ old('cust_email') }}" autocomplete="email"
                            ...
                            <div>
                                ...
                                <input id="modal_password" name="password" type="password" required
                                       ...
                                       class="w-full rounded-lg border px-3 py-2.5 md:py-2 text-sm text-gray-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 @error('password') border-red-400 bg-red-50 @else border-gray-300 @enderror">
                                @error('password')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                ...
                <div x-show="tab === 'anon'">
                    <p class="mb-5 text-sm text-gray-500 leading-relaxed">
                        Du hast von einem Galeristen ein Kurzzeit-Kennwort erhalten? Gib es hier ein.
                    </p>
                    <form method="POST" action="{{ route('customer.login.anon') }}" autocomplete="off">
                        @csrf
                        <div>
                            <label for="modal_anon_password"
                                   class="block text-sm font-medium text-gray-700 mb-1">Kurzzeit-Kennwort</label>
                            <input id="modal_anon_password" name="password" type="password" required minlength="8"
                                   autocomplete="off"
                                   class="w-full rounded-lg border px-3 py-2.5 md:py-2 text-sm text-gray-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 @error('password') border-red-400 bg-red-50 @else border-gray-300 @enderror">
                            @error('password')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
