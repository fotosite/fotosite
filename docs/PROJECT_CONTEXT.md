...
Validierung über Laravel `Password` Rule Facade. Je User-Typ eigene Regel-Instanz via `Password::defaults()` oder explizit im Request.
...
| HIBP-Prüfung (uncompromised()) | ✅ | ✅ | — |
| Benutzername im Passwort verboten | ✅ | ✅ | ✅ |

### pw_list-Passwörter (Mandant-definiert)
Passwörter in `pw_list` (pw1–pw6) sind Zugangscodes für anonyme Customer, keine User-Passwörter.
Regel: min. 8 Zeichen, keine weiteren Einschränkungen (Mandant verteilt sie selbst, zeitlich begrenzt).

### Implementierungsnotiz
Laravel-Beispiel für mand:
```php
...
### Motivation
Passkeys sind für `mand` und `cust` funktionale Notwendigkeit, kein Komfort-Feature:
- `mand`: Foto-Upload direkt vom Mobilgerät (Handy = primäre Kamera) — 2FA per Email im mobilen Workflow inakzeptabel
- `cust`: Mobile-first-Nutzung — 2FA-Eingabe auf Mobilgeräten führt zu Abbrüchen

### Strategie pro Rolle

| Rolle | Passkey | 2FA per Email | Passwort-Fallback |
|---|---|---|---|
| syst | optional (Phase 4) | ✅ primär (bereits impl.) | ✅ |
| mand | ✅ Priorität Phase 2b | ✅ Fallback ohne Passkey | ✅ |
| cust registriert | ✅ Priorität Phase 2c | ❌ entfällt | ✅ |
| anon | ❌ | ❌ | pw_list-Codes |

Sobald ein User einen Passkey registriert hat, entfällt der 2FA-Schritt — der Passkey ist der zweite Faktor (Gerätbesitz + Biometrie).

### Package
`laragear/webauthn` — kompatibel mit Laravel 13 / PHP 8.4+.
Anpassung erforderlich, da User-Tabellen nicht dem Laravel-Standard entsprechen (custom PKs, vier Datenbanken).

### id-Spalten-Lösung
Falls `laragear/webauthn` eine Spalte `id` voraussetzt, wird diese als zusätzliche Spalte (nicht PK) in `mand_user` und `cust_user` eingefügt:
...
Primary Keys `mand_id` und `cust_id` bleiben unverändert. Im Model wird `getKey()` und `getKeyName()` überschrieben, ausschließlich für WebAuthn-Kompatibilität. Alle übrigen Anwendungsteile verwenden weiterhin `mand_id` / `cust_id`.

### passkey-Tabelle (in userdb)
...
CREATE TABLE `passkey` (
    ...
    PRIMARY KEY(`pk_id`),
    UNIQUE KEY `passkey_credential_id_unique`(`credential_id`),
    INDEX `passkey_user_index`(`user_type`, `user_id`)
);
```
...
- Passkey löschen (mit Bestätigungsdialog)
- Passkey umbenennen
- Liste aller registrierten Geräte mit `last_used_at` einsehen

### Aufforderung zur Passkey-Registrierung
- `mand`: Modal nach erstem Login — muss aktiv weggeklickt werden (Option „Mittel")
- `cust`: Hinweis-Banner im Dashboard (Option „Sanft")

### Login-Modal mand (Zielzustand)
```
Mandanten-Login
├── [Mit Passkey anmelden]     ← primärer CTA
├── ──── oder ────
├── Benutzername + Passwort
│   └── → 2FA-Schritt          ← nur ohne Passkey
└── [Passwort vergessen?]
```
...
| Phase | Inhalt |
|---|---|
| Jetzt | Mandant-Login mit Passwort + 2FA |
| Phase 2a | Passkey-Infrastruktur: DDL, Package, Model-Interfaces testen |
| Phase 2b | Passkey für mand aktivieren |
| Phase 2c | Passkey für cust aktivieren |
| Phase 3 | Customer-Login Passwort-Fallback (ohne 2FA) für Desktop |
| Phase 4 | Passkey für syst (optional) |

### Git-Strategie
Feature-Branch `feature/passkey-infra` ab `system_login_2fa_ok`.
Main bleibt stabil bis zum erfolgreichen Merge.
Bei Scheitern: Branch enthält alle Erkenntnisse für zweiten Versuch — main unberührt.
