...
- **04.08. (neu, aktueller Stand):** `trusted_device_passkey_ok` (Commit `268c2b7`) — Trusted-Device-Cookie auch im Passkey-Login-Pfad wirksam (cust+mand), JS liest Checkbox-Zustand aus dem DOM, `issueTrustedDeviceIfRequested()`-Typehint auf `RedirectResponse|JsonResponse` gelockert. Details Abschnitt 10h. Danach `logout_dialog_close_window_ok` (Commit `859516d`) — Logout-Dialog-Button „Zurück" zu „Fenster schließen", `window.close()`-Versuch mit Dialog-Fallback, löst Inkonsistenz #27. Details Abschnitt 10i.
