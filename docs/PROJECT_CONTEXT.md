...
- **26.08. (neu, aktueller Stand):** `pflichtfelder_konfiguration_ok` (Commit `eef2eed`) — konfigurierbare Pflichtfelder für mand/cust (Telefon/Straße+Hausnr./PLZ+Ort/Firma) über neue Datei `storage/app/private/pflichtfelder.txt` + Helper `istPflichtfeld()`; begleitende DB-Migration `userdb.*_tel`/`*_street+nr`/`*_postcode(_city|+city)`/`*_company` auf `NULL DEFAULT NULL`; neue mand-Detailseite je Mitglied (`mandant.kunden.show`) + umgebaute Mitgliederliste; syst sieht jetzt zusätzlich mand-Adresse; Pflicht-Sternchen-Bugfix in `mandant/konto.blade.php`. Details Abschnitt 10j.
