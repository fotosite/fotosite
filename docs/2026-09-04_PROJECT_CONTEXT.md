...
> Stand: 29.08.2026 — Tag: `ui_texte_ausgelagert_ok` (letzter Commit-Tag; die folgende Ergänzung ist uncommitted). **29.08.: Vorbereitung fünfte Datenbank `subscriptiondb` für ein späteres Abrechnungssystem (mand+cust).** Neue Datenbank `u14bc1w8_v08_subscriptiondb`, Laravel-Connection `subscriptiondb`, Base-Model `App\Models\SubscriptionDb\SubscriptionDbModel` (minimal, analog den vier bestehenden Base-Models). Fünf Tabellen: `subscriber` (Vertragspartner, überdauert gelöschte Accounts), `plan` (versionierte Tarife), `subscription` (Vertrag je Nutzer — ein Subscriber kann mehrere Verträge zahlen), `ledger_entry` (unveränderliches Buchungsjournal, fünf Buchungsarten `FO`/`GG`/`ZE`/`ZG`/`ZA`, zwei getrennte Betragsspalten für Geldkonto und Vertragssaldo), `invoice` (Rechnung als Momentaufnahme je Vertrag). Kein Eingriff in bestehende Tabellen — Verknüpfung zu `mand_user`/`cust_user` ausschließlich logisch über `user_type`+`user_id`, analog dem etablierten Muster bei `passkey`/`trusted_device`. Abweichend vom sonstigen Projektstandard werden innerhalb dieser Datenbank bewusst FK-Constraints gesetzt (dokumentieren die Verwendung, alle `ON DELETE RESTRICT`). DDL für alle fünf Tabellen ausgeführt, illustrierender Demo-Datensatz eingespielt (Subscriber 30118 mit drei Verträgen, entspricht dem Buchungsbeispiel aus Konzept-Abschnitt 4.2). Trigger für die Unveränderbarkeit des Journals **noch nicht** gesetzt. Anwendungslogik bewusst NICHT implementiert — Abschnitt 4 des Konzepts beschreibt sie als Erweiterungsoption für eine mögliche spätere kommerzielle Nutzung. Nebenbei behoben: Die bereits ausgeführte DDL hatte `entry_type` fälschlich mit `'LS'` statt `'ZA'` angelegt (Bug, kein bewusster Namenswechsel) — korrigiert per `ALTER TABLE`+`UPDATE`. Details Abschnitt 10n, vollständiges Konzept: `docs/Konzept_Abrechnungssystem.md`. Noch UNCOMMITTED, kein Tag. Vorher, 27.08. — Tag: `ui_texte_ausgelagert_ok`. **27.08.: UI-/E-Mail-Texte in editierbare Markdown-Dateien ausgelagert + 2FA-Gültigkeitsdauer zentralisiert.** Neuer Helper `uiText()` (`app/helpers.php`) liest `storage/app/private/ui-texte/{all,cust,mand,syst}/*.md` (nicht versioniert, analog `honeypot_paths.txt`/`pflichtfelder.txt`) und rendert per `CommonMarkConverter` zu HTML — 28 Textstellen (Login-/2FA-Hinweise, Konto-Löschen-/Galerie-entfernen-Warnungen, Upload-Bedingungen-Hinweis, E-Mail-ändern-Erklärung, Spam-Hinweis, Passkey-Prompts, Alias-Erklärung, Pflichtfeld-Nachtrag-Hinweis, Policy-Update-Hinweise, 7 E-Mail-Bodies) ausgelagert, wortgleiche Duplikate (auch über cust/mand/syst hinweg) auf je eine gemeinsame Datei konsolidiert. `renderMarkdownVariant()` (bestehender Tag-Varianten-Mechanismus, z.B. Passkey-OS-Hinweise) um optionalen `$vars`-Parameter erweitert; `uiText()` nutzt denselben `{{key}}`-Platzhalter-Mechanismus (`str_replace` vor Markdown-Konvertierung) für Variablen mitten im Satz (E-Mail-Bodies mit Code/Name/Frist). Alle Dateien vorübergehend mit rotem `[DEV]`-Marker versehen (Test-Fortschrittsanzeige, wird nach Verifikation je Anzeigestelle entfernt). Nebenbei behoben: 2FA-Code-Gültigkeitsdauer war an zwei Stellen hartcodiert (`TwofaService::VALID_MINUTES=2`, `TwoFactorCodeMail` `'validMinutes'=>2`, potenzieller DRY-Verstoß) und in den drei Rollen-Login-Hinweistexten uneinheitlich (cust nannte die Dauer, mand/syst nicht; syst-Text war zudem noch gar nicht ausgelagert) — neue `config/twofa.php` + `TWOFA_CODE_VALID_MINUTES` in `.env` zentralisieren das, alle drei Rollen nutzen jetzt denselben Text (`all/a_log_2fa_hinweis.md`) mit dynamischem `{{validMinutes}}`. **Zusätzlich, `.env`-only (nicht versioniert, aber operational relevant):** Mail-Konfiguration auf echtes Postfach umgestellt — `MAIL_SCHEME=starttls` wurde von der installierten Symfony-Mailer-Version (8.0.8) nicht unterstützt und führte zu 500er-Fehlern bei jedem Mail-Versand (2FA-Login etc.), zusätzlich zeigte `.env` auf einen lokalen `127.0.0.1:1025`-Mailcatcher statt echtem SMTP; jetzt neues Postfach `noreply@martinwagner.de` bei Alfahosting, `MAIL_SCHEME=smtps`, `MAIL_HOST=host159.alfahosting-server.de`, `MAIL_PORT=465`, getestet cust/mand/syst. `BACKSTAGE_PATH` erstmals explizit in der Server-`.env` gesetzt (individueller Pfad statt Code-Default `backstage` aus `config/app.php`) — Sicherheitsgewinn, da der Pfad nicht mehr aus dem öffentlich einsehbaren Code ableitbar ist; frühere Referenzen auf „/backstage" in diesem Dokument sind als Default-Pfad zu lesen, nicht als aktueller Wert. Tag `ui_texte_ausgelagert_ok`, Commit `2dbeb37`. Details Abschnitt 10m. Vorher, 26.08. — Tag: `registration_policy_version_fix_ok`. **26.08.: Bugfix Policy-Version bei Registrierung.** `CustRegisterController::store()` und `SystemMandantController::handleRegister()` setzten `ds_version`/`upload_terms_version` bisher auf den statischen, nicht mit `userdb.policy_versions` synchronisierten Wert `config('datenschutz.version')` (`'1.0'`) statt auf die tatsächlich aktuelle, von `CheckPolicyVersion` geprüfte Version — dadurch erschien bei jeder Neuregistrierung sofort beim ersten Login der „Policy geändert"-Hinweis, obwohl gerade erst zugestimmt wurde. Zusätzlich bekam mand `upload_terms_version` bisher fälschlich den `ds_version`-Wert zugewiesen. Beide Felder lesen jetzt `PolicyVersion::get('ds_version')` bzw. `PolicyVersion::get('upload_version')`. Tag `registration_policy_version_fix_ok`, Commit `83c738e`. Details Abschnitt 10l. Vorher, 26.08. — Tag: `pflichtfelder_erzwingung_ok`. **26.08.: Pflichtfelder-Erzwingung nach Login.** Neue Middleware `CheckPflichtfelder` (`bootstrap/app.php`, zwischen `CheckPolicyVersion` und `CheckWelcome`) prüft bei jedem Request von cust/mand, ob ein aktuell laut `pflichtfelder.txt` als Pflicht konfiguriertes Feld (Telefon/Straße/PLZ+Ort/Firma) `NULL` ist. Falls ja: Redirect zur Konto-Seite mit `withErrors()` (Feldmarkierung wie bei fehlgeschlagener Validierung) + Amber-Hinweisbox („Ein oder mehrere Felder sind inzwischen zu zusätzlichen Pflichtfeldern geworden..."). Ausnahmen: Konto-Seite/Speichern-Route selbst, Logout, Datenschutz-Routen. 60s-Cache pro User (`Cache::forget()` beim Speichern). mand bleibt nach dem Speichern auf der Konto-Seite; cust wird zur ursprünglich angeforderten Seite zurückgeleitet (Session-Key `pflichtfeld_redirect_target`, nur für cust gesetzt). Zusätzlich: grauer „(optional)"-Hinweis bei Straße/PLZ+Ort ergänzt (hatten bisher nur das bedingte Sternchen, siehe Abschnitt 10j). Tag `pflichtfelder_erzwingung_ok`, Commit `16e81f3`. Details Abschnitt 10k. Vorher, 26.08. — Tag: `pflichtfelder_konfiguration_ok`. **26.08.: Konfigurierbare Pflichtfelder für mand/cust (Telefon/Straße+Hausnr./PLZ+Ort/Firma) eingeführt.** Neue Datei `storage/app/private/pflichtfelder.txt` (nicht versioniert, wie `honeypot_paths.txt`) steuert je Feld `mand` (Pflicht) oder `opt` (optional) über neuen Helper `istPflichtfeld(userType, feldKey)` in `app/helpers.php`; fehlender Eintrag defaultet zu `opt` (fail-safe). Registrierung (`CustRegisterController`, `SystemMandantController::handleRegister()`) fragt nur Pflichtfelder ab — optionale Felder werden im Formular komplett ausgeblendet (`@if(istPflichtfeld(...))`); Konto-Bearbeiten (`CustSelfController`, `MandantSelfController::update()`) zeigt immer alle vier Felder, Pflicht-Status dynamisch (`required`/rotes Sternchen bzw. `nullable`/kein Sternchen). Startkonfiguration (Telefon+Firma optional, Straße+PLZ/Ort Pflicht, bei cust UND mand) bildet das bisherige Verhalten unverändert ab. **Begleitende DB-Migration:** `cust_tel`/`cust_street+nr`/`cust_postcode_city`/`cust_company` sowie die vier mand-Pendants in `userdb` per direktem SQL (phpMyAdmin) von `NOT NULL DEFAULT 'nicht vorhanden'` auf `NULL DEFAULT NULL` umgestellt, bestehende Platzhalterwerte per UPDATE auf echtes NULL migriert; Anzeige-Fallback bleibt überall `?? 'nicht vorhanden'`, für den Nutzer unverändert. `mand_uname`/`cust_uname` bleiben unverändert immer Pflicht. **Nebenbei:** `mandant/konto.blade.php` fehlende rote Pflicht-Sternchen bei `mand_uname`/`mand_firstname`/`mand_lastname`/`mand_street+nr`/`mand_postcode+city` ergänzt (Bugfix). syst sieht jetzt zusätzlich mand-Straße/PLZ+Ort (`system/mandanten/show.blade.php` + `edit.blade.php`, neue dt/dd-Paare, Feldreihenfolge umgestellt). **Neu für mand:** Read-only Detailseite je Mitglied (`mandant/cust/show.blade.php`, Route `GET /mandant/kunden/{id}` → `mandant.kunden.show`, `MandantCustController::show()`, Sicherheitscheck `pcode_id`+`mand_id` sonst `abort(404)`) zeigt erstmals Telefon/Firma/Adresse eigener Mitglieder (vorher nirgends sichtbar); Mitgliederliste (`mandant/cust/index.blade.php`) umgebaut — E-Mail (Link zur Detailseite, `text-indigo-600 underline`) + `cust_uname` in erster Spalte/Zeile statt Alias+E-Mail, Alias-Bearbeitung von festem Input auf Anzeige-Text mit Bearbeiten/Speichern-Toggle-Button (`x-data="{ editing: false }"` pro Zeile). Getestet: cust/mand Registrieren+Bearbeiten, syst-Anzeige, mand-Mitgliederliste (Toggle, Links, Sortierung/Suche) — alles bestätigt erfolgreich. Tag `pflichtfelder_konfiguration_ok`, Commit `eef2eed`. Details Abschnitt 10j. Vorher, 04.08. — Tag: `logout_dialog_close_window_ok`. **04.08.: Zwei kleine Fixes.** (1) Trusted-Device-Cookie ist jetzt auch im Passkey-Login-Pfad wirksam (cust+mand) — die `remember_device`-Checkbox liegt außerhalb des Passwort-`<form>` und wurde vom Passkey-`fetch()` bisher nicht mitgesendet; JS liest den Checkbox-Zustand jetzt direkt aus dem DOM, `issueTrustedDeviceIfRequested()` in beiden `passkeyLogin()`-Methoden ergänzt (Typehint auf `RedirectResponse|JsonResponse` gelockert). Tag `trusted_device_passkey_ok`, Commit `268c2b7`. Details Abschnitt 10h. (2) Logout-Dialog-Button „Zurück" zu „Fenster schließen" umbenannt, zusätzlicher `window.close()`-Versuch mit Dialog-Schließen als Fallback — löst Inkonsistenz #27 (Abschnitt 12). Tag `logout_dialog_close_window_ok`, Commit `859516d`. Details Abschnitt 10i. Vorher, 03.08. — Tag: `trusted_device_cust_nofactor_fix_ok`. **03.08.: Bugfix** — im cust-Nicht-2FA-Login-Pfad wurde die `remember_device`-Checkbox ausgelesen, aber nie ausgewertet; `issueTrustedDeviceCookie()` wurde dort nie aufgerufen (analoger mand-Pfad war bereits korrekt). Behoben durch neue Methode `CustLoginController::issueTrustedDeviceIfRequested()`, analog `MandantLoginController`. Details Abschnitt 10g. Vorher, 01.08. — Tag: `anon_share_link_shortcode_ok`. Seit 19.07.: umfangreiche Sicherheits-Härtung des Login-Systems — Fehlermeldungen dirty-ausgeblendet + Error-Bag-isoliert (`error_messages_dirty_fix_ok`), Login-Cleanup um `invite`/`cust_invite`/`trusted_device`/`twofa_code` erweitert (`login_cleanup_expired_records_ok`), Passkey-Hinweistexte in editierbare md-Dateien ausgelagert (`passkey_hints_markdown_ok`), Platzhaltertexte in Einladungsformularen präzisiert (`invite_placeholder_texts_ok`), mand-2FA optional deaktivierbar (`mand_2fa_optin_login_fix_ok`), Seitenkopf-Label „MITGLIED" ergänzt + Inkonsistenz #12 (`passkey.sign_count`) dokumentiert (`mitglied_label_und_inkonsistenz12_ok`), einheitliche IP-basierte Login-Sperre für cust/mand/syst (`login_lockout_ip_based_ok`), syst-Login-Pfad über `.env` konfigurierbar (`backstage_path_configurable_ok`), Log-Kanal `login_attacks` + dynamische Honeypot-Routen (`honeypot_login_attacks_log_ok`). **Zusätzlich, zum Zeitpunkt dieses Doku-Standes noch UNCOMMITTED (kein Tag):** syst-Passwort-Policy auf min. 20 Zeichen + Groß-/Kleinbuchstaben + Ziffer + Sonderzeichen verschärft, inkl. Hard-Block beim Login bei nicht mehr konformem Bestandspasswort (kein Self-Service-Reset für syst). Details Abschnitt 8/8a. Phase 7 (Foto-Content) und der gründliche Passkey-Gesamttest sind weiterhin die fachlich nächsten Schritte — siehe unten.
>
...
- **Mail:** SMTP `host159.alfahosting-server.de:465`, `MAIL_SCHEME=smtps` (SSL/TLS), eigenes Postfach `noreply@martinwagner.de` bei Alfahosting. `MAIL_FROM_NAME="Fotosite"`. **Korrektur 27.08.:** `.env` zeigte bis dahin fälschlich auf einen lokalen `127.0.0.1:1025`-Mailcatcher mit `MAIL_SCHEME=starttls` — von der installierten Symfony-Mailer-Version (8.0.8) nicht unterstützt, führte zu 500er-Fehlern bei jedem Mail-Versand (2FA-Login etc.). Nach Umstellung getestet cust/mand/syst. Details Abschnitt 10m.
...
Fünf separate MariaDB-Datenbanken, jede mit eigener Laravel-Connection und eigenem DB-User (`DB_<CONNNAME>_*` in `.env`). **Keine Cross-DB-Joins** — Verknüpfung ausschließlich über `mand_id` / `cust_id` / `fo_id` in PHP.

> **Echte Foreign Keys:** `fotodb.activity_subgroup.ag_id → fotodb.activity_group.ag_id` sowie `userdb.cust_pcode.mand_id → userdb.mand_user.mand_id`. Alle anderen Verknüpfungen sind logisch, ohne FK-Constraint. **Ausnahme `subscriptiondb` (NEU, 29.08.):** Dort werden innerhalb der Datenbank bewusst mehrere FK-Constraints gesetzt (`subscription`→`subscriber`/`plan`, `ledger_entry`→`subscriber`/`subscription`/Selbstreferenz, `invoice`→`subscriber`/`subscription`/`ledger_entry`) — bewusste Abweichung vom sonstigen Projektstandard, dokumentiert die Verwendung der Datenbank. Siehe Abschnitt 4.5 und `docs/Konzept_Abrechnungssystem.md` Abschnitt 5.4.
...
| `mand_user` | `mand_id` | Galerist:in-Accounts (inkl. `mand_pw_hash`, `mand_cust_2fa`, `mand_2fa_opt_in`, `has_public_content`, `active`, `valid_to` DATE NULL (Option bei Zahlungsausfall — Nutzung noch offen), `ds_accepted_at`, `ds_version`, `upload_terms_accepted_at`, `upload_terms_version`, `show_welcome` TINYINT(1) Default 1). `mand_tel`/`mand_street+nr`/`mand_postcode+city`/`mand_company` **seit 26.08. NULL DEFAULT NULL** (vorher NOT NULL DEFAULT 'nicht vorhanden') — Pflicht-Status jetzt konfigurierbar über `storage/app/private/pflichtfelder.txt` (`istPflichtfeld()`), Anzeige-Fallback weiterhin `?? 'nicht vorhanden'`. Details Abschnitt 10j |
| `cust_user` | `cust_id` | Mitglieder-Accounts (inkl. `cust_2fa_opt_in` TINYINT(1) Default 0 — Funktion noch zu klären, `ds_accepted_at`, `ds_version`, `upload_terms_accepted_at`, `upload_terms_version` — Felder strukturell vorhanden, Upload-Bedingungen derzeit für cust nicht relevant, `show_welcome` TINYINT(1) Default 1). `cust_tel`/`cust_street+nr`/`cust_postcode_city`/`cust_company` **seit 26.08. NULL DEFAULT NULL** (vorher NOT NULL DEFAULT 'nicht vorhanden') — Pflicht-Status jetzt konfigurierbar über `storage/app/private/pflichtfelder.txt` (`istPflichtfeld()`), Anzeige-Fallback weiterhin `?? 'nicht vorhanden'`. Details Abschnitt 10j |
...
### 4.5 `subscriptiondb` — Abrechnungssystem (vorbereitet, NEU 28.–29.08.2026)
Connection: `subscriptiondb`

| Table | PK | Purpose |
|---|---|---|
| `subscriber` | `sr_id` | Vertragspartner (wer zahlt), getrennt vom Login-Datensatz — überdauert einen gelöschten Account (Aufbewahrungspflicht), ein Subscriber kann mehrere Verträge für mehrere Nutzer bündeln |
| `plan` | `pl_id` | Tarif, versioniert — jede Zeile eine unveränderliche Version (`pl_code`+`pl_version` UNIQUE), Preisänderung = neue Zeile |
| `subscription` | `sb_id` | Vertrag je Nutzer über die gesamte Laufzeit (`sr_id`, `user_type` enum(mand,cust), `user_id` — logische Referenz, `pl_code`, `pl_version_current`, `sb_status`) |
| `ledger_entry` | `le_id` | Unveränderliches Buchungsjournal. `entry_type` enum(`FO`,`GG`,`ZE`,`ZG`,`ZA`) — Forderung/Gewährte Gutschrift/Zahlungseingang/Zahlungsgutschrift/Zahlungsausgang. Zwei getrennte Betragsspalten: `money_amount` (Geldkonto) und `amount` (Vertragssaldo). Bewusst kein `updated_at` |
| `invoice` | `re_nr` | Rechnung als Momentaufnahme je Subscription, kein eigener Forderungsträger — der ergibt sich aus `ledger_entry` zwischen `from_le_id` und `to_le_id` |

**Bewusste Abweichungen vom Projektstandard:** Innerhalb dieser Datenbank werden echte FK-Constraints gesetzt (alle `ON DELETE RESTRICT`) statt der sonst üblichen rein logischen Verknüpfung — siehe Callout am Anfang von Abschnitt 4. Verknüpfung zu `userdb.mand_user`/`userdb.cust_user` bleibt trotzdem ausschließlich logisch über `user_type`+`user_id` (analog `passkey`/`trusted_device`), da datenbankübergreifende FKs technisch nicht möglich sind.

**Stand 29.08.2026:** DDL für alle fünf Tabellen ausgeführt, Demo-Datensatz eingespielt (Subscriber 30118, drei Verträge). Trigger für Journal-Unveränderbarkeit noch nicht gesetzt. Keine Tabellen-Models, keine Anwendungslogik. Details Abschnitt 10n, vollständiges Konzept `docs/Konzept_Abrechnungssystem.md`.
...
│   └── SubscriptionDb/ (SubscriptionDbModel)
│                    # NEU 28.–29.08. — nur Base-Model, keine Tabellen-Models
│                    # (Feldlisten stehen fest, Models noch nicht angelegt)
├── Mail/
│   ├── InviteMail.php
│   ├── CustInviteMail.php
...
└── twofa.php            # NEU 27.08. — 'valid_minutes' => env('TWOFA_CODE_VALID_MINUTES', 2)
...
**KORREKTUR/ERGÄNZUNG 26.08.:** Der Pflicht/Optional-Status von `*_tel`/`*_street+nr`/`*_postcode+city`/`*_company` ist seit `pflichtfelder_konfiguration_ok` **konfigurierbar** (`storage/app/private/pflichtfelder.txt`, Helper `istPflichtfeld()`) statt fest im Code verdrahtet — die hier beschriebene Aufteilung (Straße/PLZ+Ort Pflicht, Telefon/Firma optional) ist die aktuelle Startkonfiguration, kein Codefixum mehr. Details Abschnitt 10j.
...
## 10h. Passkey-Login: Trusted-Device-Cookie nachgezogen (cust+mand) — NEU, 04.08.2026 (`trusted_device_passkey_ok`)

**Bug:** Die Checkbox „Dieses Gerät als sicher merken" wirkte bisher nur beim Passwort-Login. Der Passkey-Button (`cust-passkey-btn`/`passkey-btn` in `login-modal.blade.php`) liegt außerhalb des Passwort-`<form>` und löst per `onclick` einen reinen `fetch()`-Request aus, kein Formular-Submit — der Checkbox-Wert (Alpine-State `rememberDevice`, an das Passwort-Form gebunden) erreichte den Server auf diesem Weg nie. Entsprechend riefen `CustLoginController::passkeyLogin()` und `MandantLoginController::passkeyLogin()` `issueTrustedDeviceIfRequested()` nie auf — Passkey-Logins legten trotz gesetzter Checkbox keinen Trusted-Device-Datensatz an.

**Fix:**
- JS (`login-modal.blade.php`, `loginWithCustPasskey()`/`loginWithPasskey()`): liest den Checkbox-Zustand direkt aus dem DOM (`document.getElementById('remember_device_cust'/'_mand')?.checked ?? false`) und sendet ihn als zusätzliches Feld `remember_device` im JSON-Body des Passkey-`fetch()`-Requests mit.
- `CustLoginController::passkeyLogin()`: lädt jetzt zusätzlich `$cust = CustUser::find($userId)` (vorher nicht benötigt, da nur `$userId` verwendet wurde), liest `$rememberDevice = $request->boolean('remember_device')` (Laravel liest `boolean()`/`input()` bei `Content-Type: application/json` automatisch aus dem JSON-Body) und ruft nach dem Session-Aufbau `issueTrustedDeviceIfRequested()` auf.
- `MandantLoginController::passkeyLogin()`: identische Ergänzung (`$mand` war dort bereits geladen).
- `issueTrustedDeviceIfRequested()` in beiden Controllern erwartete bisher zwingend eine `RedirectResponse` (für `->cookie()`); `passkeyLogin()` gibt aber eine `JsonResponse` zurück. Typehint auf `RedirectResponse|JsonResponse` gelockert — beide Klassen nutzen intern dasselbe `Illuminate\Http\ResponseTrait` und stellen `->cookie()` identisch bereit, kein Interface/Base-Class-Umbau nötig.
- Mail-Versand (`TrustedDeviceAddedMail`) läuft unverändert mit, da beide Pfade dieselbe, bereits bestehende `issueTrustedDeviceIfRequested()`-Methode wiederverwenden.

Getestet: cust + mand, Trusted-Device-Datensatz (`sessiondb.trusted_device`) und Mail-Versand nach Passkey-Login mit gesetzter Checkbox bestätigt.

**Tag:** `trusted_device_passkey_ok`. Commit: `268c2b7`.

---

## 10i. Logout-Dialog: „Zurück"-Button zu „Fenster schließen" — NEU, 04.08.2026 (`logout_dialog_close_window_ok`)

Im Trusted-Device-Bestätigungsdialog (`resources/views/components/logout-button.blade.php`) wurde der bisherige „Zurück"-Button (schloss nur den Alpine-Dialog, `showConfirm = false`) umbenannt zu „Fenster schließen" und ruft zusätzlich `window.close()` auf: `@click="window.close(); showConfirm = false"`. `window.close()` funktioniert per Browser-Vorgabe nur bei Tabs, die per Skript geöffnet wurden — bei anderen Tabs wird der Aufruf stillschweigend ignoriert (keine Exception, JS kann das Scheitern nicht zuverlässig erkennen). Der Klick dient in diesem Fall als Aufforderung an den Nutzer, das Fenster selbst zu schließen. `showConfirm = false` bleibt unverändert als Fallback bestehen — der Dialog schließt sich in jedem Fall wie bisher, unabhängig davon, ob `window.close()` gewirkt hat. Der bestehende Alpine-Mechanismus (`x-data="{ showConfirm: false }"`, `x-show`, `x-cloak`, `@click.outside`) ist unverändert.

Löst Inkonsistenz #27 (Abschnitt 12) sowie Punkt 5 der offenen Aufgaben (Abschnitt 16a) — dort jeweils als behoben markiert. Umgesetzter Text weicht von der ursprünglich besprochenen, dann abgebrochenen Idee „Verlassen ohne Löschen" ab; das Ergebnis behält die bisherige Dialog-Semantik bei (kein „ohne Löschen abmelden", nur Fenster-schließen-Versuch + Fallback).

**Tag:** `logout_dialog_close_window_ok`. Commit: `859516d`.

---

## 10j. Konfigurierbare Pflichtfelder für mand/cust (Telefon/Straße/PLZ+Ort/Firma) — NEU, 26.08.2026 (`pflichtfelder_konfiguration_ok`)

### Pflichtfelder-Konfiguration
Neue Datei `storage/app/private/pflichtfelder.txt` (nicht versioniert, analog `honeypot_paths.txt`) steuert je Feld, ob es für mand bzw. cust Pflicht oder optional ist. Format: `M_Telefon=opt`, `C_Strasse=mand` etc. (Präfix `M_`/`C_` + `Telefon`|`Strasse`|`PLZOrt`|`Firma`, Wert `mand` oder `opt`), 8 Zeilen. Fehlt die Datei oder ein Eintrag, gilt `opt` (fail-safe). Gelesen über neuen Helper `istPflichtfeld(string $userType, string $feldKey): bool` in `app/helpers.php`.

**Startkonfiguration:** Telefon + Firma = optional, Straße + PLZ/Ort = Pflicht (bei cust UND mand) — bildet das bisherige, vorher fest im Code verdrahtete Verhalten unverändert ab (siehe Abschnitt 10b, KORREKTUR 26.08.).

### Registrierung — nur Pflichtfelder abfragen
`CustRegisterController::store()` (existing=0-Zweig) und `SystemMandantController::handleRegister()`: `validate()`-Regeln für die vier Felder werden per Schleife über `istPflichtfeld()` dynamisch aufgebaut — optionale Felder fehlen komplett im Rules-Array. In den Blade-Formularen (`customer/auth/register.blade.php`, `system/mandanten/register.blade.php`) ist jedes der vier Felder in `@if(istPflichtfeld('cust'|'mand', 'Telefon'|'Strasse'|'PLZOrt'|'Firma')) ... @endif` gewickelt — bei `opt` wird das Feld komplett nicht gerendert, nicht nur ausgeblendet. Beim Anlegen (`CustUser::create()`/`MandUser::create()`) wird für nicht abgefragte (optionale) Felder explizit `null` gespeichert statt des früheren Platzhalter-Strings `'nicht vorhanden'`.

### Konto-Bearbeiten — immer alle Felder, Pflicht-Status dynamisch
`CustSelfController::update()` und `MandantSelfController::update()`: alle vier Felder bleiben immer im `validate()`-Array (werden im Formular immer angezeigt), Regel wird pro Feld `required` oder `nullable` je nach `istPflichtfeld()`. In `customer/konto.blade.php`/`mandant/konto.blade.php` sind `required`-Attribut UND rotes Pflicht-Sternchen (`<span class="text-red-500">*</span>`) jeweils per `@if(istPflichtfeld(...))` konditional; bei Telefon/Firma ist zusätzlich der graue „(optional)"-Hinweistext per `@if(! istPflichtfeld(...))` konditional (verhindert Widerspruch „*" + „(optional)" gleichzeitig, falls die Konfiguration künftig wechselt). Der bisherige Fallback-Code (`$validated['cust_tel'] ?? 'nicht vorhanden'`) wurde entfernt — leere optionale Felder werden als `null` gespeichert, da Laravels Standard-Middleware `ConvertEmptyStringsToNull` leere Strings bereits vor der Validierung in `null` umwandelt.

### DB-Migration: echtes NULL statt Platzhalter-String
`cust_tel`, `cust_street+nr`, `cust_postcode_city`, `cust_company` sowie `mand_tel`, `mand_street+nr`, `mand_postcode+city`, `mand_company` in `userdb` per direktem SQL (phpMyAdmin, keine Laravel-Migration — siehe Abschnitt 4) von `NOT NULL DEFAULT 'nicht vorhanden'` auf `NULL DEFAULT NULL` umgestellt (`ALTER TABLE ... MODIFY ...`); bestehende `'nicht vorhanden'`-Platzhalterwerte per `UPDATE` auf echtes `NULL` migriert. Die Anzeige bleibt für den Nutzer unverändert: überall dort, wo diese Felder ausgegeben werden, sorgt ein `?? 'nicht vorhanden'`-Fallback im Blade-Code für denselben Text wie vorher, jetzt aber DB-seitig als sauberes `NULL` statt Platzhalter-String. `mand_uname`/`cust_uname` sind von alldem unberührt — bleiben unverändert immer Pflicht, nicht Teil dieser Konfiguration.

### Bugfix: mandant/konto.blade.php fehlende Pflicht-Sternchen
`mand_uname`, `mand_firstname`, `mand_lastname`, `mand_street+nr`, `mand_postcode+city` hatten in `mandant/konto.blade.php` nie ein sichtbares rotes Pflicht-Sternchen — im Gegensatz zu allen anderen Konto-Formularen im Projekt, obwohl diese fünf Felder serverseitig fest `required` sind (nicht an `istPflichtfeld()` gebunden). Bei dieser Gelegenheit ergänzt.

### syst: neue Sichtbarkeit für mand-Adresse
`system/mandanten/show.blade.php` + `edit.blade.php` zeigten bisher nur Telefon/Firma von mand, keine Adresse. Zwei neue dt/dd-Paare (Straße+Hausnr., PLZ+Ort) ergänzt; Feldreihenfolge im Profildaten-Block umgestellt auf Benutzername/E-Mail/Vorname/Nachname/Straße/PLZ+Ort/Telefon/Firma. Alle vier optionalen Felder mit `?? 'nicht vorhanden'`-Fallback in der Anzeige (können jetzt echtes NULL enthalten).

### mand: neue Detailseite je Mitglied (cust-Felder vorher gar nicht sichtbar)
Neue Route `GET /mandant/kunden/{id}` (Name `mandant.kunden.show`, `routes/mandant.php`, registriert nach den `/kunden/einladen`-Routen und vor den PATCH/DELETE-`{id}`-Routen — Reihenfolge relevant, da `{id}` sonst `GET /kunden/einladen` verschlucken würde). `{id}` ist `pcode_id`, konsistent mit den bestehenden `kunden.passcode`/`kunden.destroy`-Routen. Neue Methode `MandantCustController::show()`: Sicherheitscheck `CustPcode::where('pcode_id', $id)->where('mand_id', $mandId)`, bei Fehlschlag `abort(404)` (nie `403`, Projektkonvention). Neue, rein lesende View `mandant/cust/show.blade.php` (Stil/Aufbau analog `system/mandanten/show.blade.php`, indigo statt amber, kein Bearbeiten-Formular): zeigt `cust_alias` (mand's privater Merkname), E-Mail, Vorname, Nachname, Straße+Hausnr., PLZ+Ort, Telefon, Firma — damit kann mand erstmals Telefon/Firma/Adresse eigener Mitglieder überhaupt einsehen (vorher nirgends in der UI sichtbar, auch nicht in der Mitgliederliste).

Mitgliederliste (`mandant/cust/index.blade.php`) im selben Zug umgebaut: erste Spalte/Zeile zeigt jetzt E-Mail (Link zur neuen Detailseite, `class="font-medium text-indigo-600 underline hover:text-indigo-800"` — bewusst auch ohne Hover als Link erkennbar) + `cust_uname` darunter, statt vorher Alias + E-Mail. Alias-Bearbeitung im „Bearbeiten"-Bereich von festem Eingabefeld auf Anzeige-Text mit Bearbeiten/Speichern-Toggle-Button umgestellt: lokaler Alpine-Scope `x-data="{ editing: false }"` pro Zeile umschließt Alias-Anzeige/-Eingabe + Button (nicht das ganze `<form>`); Button-Typ wechselt `:type="editing ? 'submit' : 'button'"`, Klick im Anzeige-Modus setzt `editing = true` + `preventDefault()`. Mobile-Besonderheit: der Bearbeiten/Speichern-Button sitzt (wie schon vorher der reine „Speichern"-Button) außerhalb des `<form>` und ist nur per HTML5-`form="..."`-Attribut gebunden — der `editing`-Scope sitzt dort auf der Karten-`<div>` als gemeinsamem Vorfahren von Formular-Inhalt und Button. sec_level-Dropdown und Formular-Handler (CSRF, `$store.unsavedGuard`) unverändert; Sortier-/Suchfunktion (`custListState`, arbeitet auf Alias/E-Mail) ebenfalls unverändert funktionsfähig.

### Getestet
cust Registrieren + Konto-Bearbeiten, mand Registrieren (per Einladung) + Konto-Bearbeiten, syst-Anzeige (show + edit), mand-Mitgliederliste (Alias-Toggle, Detail-Links, Sortierung/Suche weiterhin funktionsfähig) — alles bestätigt erfolgreich.

**Tag:** `pflichtfelder_konfiguration_ok`. Commit: `eef2eed`.

---

## 10k. Pflichtfelder-Erzwingung nach Login — NEU, 26.08.2026 (`pflichtfelder_erzwingung_ok`)

Ergänzt Abschnitt 10j: die Pflichtfeld-Konfiguration (`pflichtfelder.txt`) wirkte bisher nur bei Registrierung und beim (freiwilligen) Bearbeiten des Kontos — ein Nutzer mit bereits fehlendem, neu zur Pflicht gewordenem Feld wurde nicht aktiv aufgefordert, es nachzutragen.

**Middleware:** Neue `CheckPflichtfelder` (`bootstrap/app.php`, in der Middleware-Kette zwischen `CheckPolicyVersion` und `CheckWelcome` registriert), greift bei jedem Request von cust/mand. Prüft per `istPflichtfeld()`, ob eines der vier Felder (Telefon/Straße/PLZ+Ort/Firma) aktuell als Pflicht konfiguriert ist UND in der DB `NULL` steht. Trifft das zu: Redirect zur jeweiligen Konto-Seite mit `withErrors()` (identische Feldmarkierung wie bei einer fehlgeschlagenen Validierung) sowie einer Amber-Hinweisbox („Ein oder mehrere Felder sind inzwischen zu zusätzlichen Pflichtfeldern geworden. Bitte fülle die mit einem Stern (*) gekennzeichneten Felder jetzt aus.").

**Ausnahmen** (kein Redirect-Loop): die Konto-Seite und ihre Speichern-Route selbst, Logout, sowie die Datenschutz-Routen (`*.datenschutz.*`, analog den bestehenden Gate-Middlewares `CheckPolicyVersion`/`CheckWelcome`, siehe Inkonsistenz #15/#16).

**Performance:** Prüfergebnis 60 Sekunden pro User gecacht (`Cache::remember()`), `Cache::forget()` beim Speichern des Kontos — vermeidet eine DB-Abfrage pro Request, ohne dass ein gerade nachgetragenes Feld für bis zu 60s weiterhin als fehlend gilt.

**Redirect-Ziel nach dem Ausfüllen:** mand bleibt nach dem Speichern schlicht auf der Konto-Seite (kein weiteres Ziel bekannt). cust wird zur ursprünglich angeforderten Seite zurückgeleitet — dazu wird das Zielrouten vor dem Redirect in der Session unter `pflichtfeld_redirect_target` abgelegt (nur für cust gesetzt, da mand ohnehin auf der Konto-Seite bleibt).

**Zusätzlich:** grauer „(optional)"-Hinweistext bei Straße/PLZ+Ort ergänzt — die beiden Felder hatten bisher nur das bedingte rote Pflicht-Sternchen (`@if(istPflichtfeld(...))`), aber keinen Gegenpart für den Fall `opt` (im Gegensatz zu Telefon/Firma, die diesen Hinweis bereits seit Abschnitt 10j hatten).

**Tag:** `pflichtfelder_erzwingung_ok`. Commit: `16e81f3`.

---

## 10l. Bugfix: Policy-Version bei Registrierung aus `policy_versions` statt Config gelesen — NEU, 26.08.2026 (`registration_policy_version_fix_ok`)

**Bug:** `CustRegisterController::store()` und `SystemMandantController::handleRegister()` setzten `ds_version`/`upload_terms_version` beim Anlegen eines neuen Accounts bisher auf `config('datenschutz.version')` — einen statischen Config-Wert (`'1.0'`), der nicht mit der tatsächlich maßgeblichen Version in `userdb.policy_versions` (von `CheckPolicyVersion` bei jedem Login geprüft, syst-verwaltet) synchronisiert war. Sobald `policy_versions` gegenüber `'1.0'` weitergezählt wurde (was im Projektverlauf bereits geschehen war), bekam jeder frisch registrierte Account sofort beim allerersten Login das „Policy geändert"-Popup zu sehen — obwohl der Nutzer der aktuellen Version gerade erst im Registrierungsformular zugestimmt hatte. Zusätzlich vertauscht: mand bekam für `upload_terms_version` fälschlich denselben Wert wie `ds_version` zugewiesen (statt der separaten Upload-Bedingungen-Version).

**Fix:** Beide Felder lesen jetzt `PolicyVersion::get('ds_version')` bzw. `PolicyVersion::get('upload_version')` — dieselbe Quelle, die auch `CheckPolicyVersion` für den Vergleich heranzieht. Betrifft beide Registrierungswege (cust-Selbstregistrierung per Einladung, mand-Registrierung durch syst).

**Tag:** `registration_policy_version_fix_ok`. Commit: `83c738e`.

---

## 10m. UI-/E-Mail-Texte nach editierbaren Markdown-Dateien ausgelagert + 2FA-Gültigkeitsdauer konfigurierbar — NEU, 27.08.2026 (`ui_texte_ausgelagert_ok`)

### Neuer Helper `uiText()`
Analog zum bestehenden Muster bei Passkey-Hinweisen (`renderMarkdownVariant()`, Abschnitt 10e) und der Willkommensseite (`WelcomeScreenController::renderMarkdown()`): neuer genereller Helper `uiText(string $bereich, string $dateiname, array $vars = []): string` in `app/helpers.php`. Liest `storage/app/private/ui-texte/{bereich}/{dateiname}.md` (Bereiche: `all`/`cust`/`mand`/`syst`; Verzeichnis **nicht versioniert**, analog `honeypot_paths.txt`/`pflichtfelder.txt` — Textänderungen sind damit ohne Code-Deploy per WinSCP möglich), rendert per `CommonMarkConverter` zu HTML. Fehlt die Datei, kein stiller Fail, sondern ein auffälliger roter Platzhalter `[FEHLENDER UI-TEXT: ...]` im Frontend (Tippfehler im Dateinamen fällt sofort auf). Optionaler `$vars`-Parameter ersetzt `{{key}}`-Platzhalter im Markdown-Rohtext per `str_replace` VOR der Konvertierung — erlaubt Variablen mitten im Satz (z.B. „Der Code ist `{{validMinutes}}` Minuten gültig."). `renderMarkdownVariant()` (Tag-basierte Varianten-Auswahl, z.B. Passkey-OS-Hinweise) erhielt denselben `$vars`-Mechanismus, für Fälle, in denen mehrere Textblöcke (Varianten oder durch Button/Link getrennte Absätze) in einer Datei zusammengehören.

### 28 Textstellen ausgelagert
Login-/2FA-Hinweise (cust/mand/syst — nach Konsolidierung wortgleich, eine gemeinsame Datei `all/a_log_2fa_hinweis.md`), Konto-Löschen-/Galerie-entfernen-Warnungen (inkl. Sonderfall JS-`confirm()`-Dialog in `galerien.blade.php` — dort `strip_tags()` statt HTML-Ausgabe), Upload-Bedingungen-Hinweis, E-Mail-ändern-Erklärung (cust/mand/syst zusammengeführt, war wortgleich), Spam-Hinweis, Passkey-Prompts (OS-Varianten via `renderMarkdownVariant()`), Alias-Erklärung (Einladungsformular), Pflichtfeld-Nachtrag-Hinweis (Abschnitt 10k), zwei Policy-Update-Hinweise, sowie 7 E-Mail-Bodies (Account gelöscht cust+mand, Cust-Invite, Invite/PW-Reset mit 4 Rollen-Varianten, E-Mail-Änderung, 2FA-Code, Trusted-Device-Hinweis). Wortgleiche Duplikate an mehreren Code-Stellen wurden konsequent auf je eine gemeinsame Datei konsolidiert statt separat gepflegt. Alle 28 Dateien vorübergehend mit rotem `<span style="color:red">[DEV]</span>`-Marker (bei Tag-Varianten-Dateien je Tag-Block ein eigener Marker) als Test-Fortschrittsanzeige versehen — wird nach Verifikation der jeweiligen Anzeigestelle wieder entfernt.

### 2FA-Code-Gültigkeitsdauer zentralisiert
Bei der Recherche für den 2FA-Hinweistext fiel auf: die Gültigkeitsdauer war an zwei unabhängigen Stellen hartcodiert — `TwofaService::VALID_MINUTES = 2` (steuert die tatsächliche `tfa_expires_at`-Berechnung) UND separat `TwoFactorCodeMail::content()` mit `'validMinutes' => 2` (nur für die Textanzeige in der E-Mail, ohne Bezug zur ersten Stelle) — ein DRY-Verstoß, der bei künftiger Änderung zu einer falsch angezeigten Gültigkeitsdauer hätte führen können. Zusätzlich war der Hinweistext selbst uneinheitlich: nur der cust-Login-Text nannte die Dauer überhaupt, mand nicht, und der syst-Text (in `system/login.blade.php`, 2FA als `x-show`-Zustand innerhalb der Login-Seite statt eigener View) war zu diesem Zeitpunkt noch gar nicht ausgelagert.

**Neu:** `config/twofa.php` (`'valid_minutes' => (int) env('TWOFA_CODE_VALID_MINUTES', 2)`), neue `.env`-Variable `TWOFA_CODE_VALID_MINUTES=2`. `TwofaService::VALID_MINUTES` entfernt (war nur an der einen Stelle referenziert), `TwoFactorCodeMail` liest jetzt ebenfalls `config('twofa.valid_minutes')`. Alle drei Rollen-Hinweistexte auf denselben, vollständigen Wortlaut vereinheitlicht und zu `all/a_log_2fa_hinweis.md` zusammengeführt, mit dynamischem `{{validMinutes}}`-Platzhalter statt fester bzw. fehlender Zahl.

### `.env`-only Änderungen (nicht versioniert, kein Tag, aber operational relevant)
1. **Mail-Konfiguration korrigiert** — siehe Abschnitt 3 (Server Configuration). `MAIL_SCHEME=starttls` wurde von der installierten Symfony-Mailer-Version (8.0.8, `composer show symfony/mailer`) nicht unterstützt und führte zu 500er-Fehlern bei jedem Mail-Versand; zudem zeigte `.env` auf einen lokalen `127.0.0.1:1025`-Mailcatcher statt echtem SMTP. Neues Postfach `noreply@martinwagner.de` bei Alfahosting angelegt, jetzt `MAIL_SCHEME=smtps`, `MAIL_HOST=host159.alfahosting-server.de`, `MAIL_PORT=465`. Getestet cust/mand/syst.
2. **`BACKSTAGE_PATH` erstmals explizit gesetzt** — siehe Abschnitt 10e (`backstage_path_configurable_ok`), dort ergänzt. Individueller Pfad statt Code-Default `backstage`; frühere Referenzen auf „/backstage" sind ab sofort als Default-Pfad zu lesen, nicht als aktueller Wert.
...
## 10n. Vorbereitung fünfte Datenbank `subscriptiondb` für ein Abrechnungssystem — 28.–29.08.2026 (Tag subscriptiondb_vorbereitet_ok)

Vorbereitung einer Schnittstelle/Datenstruktur für ein späteres Abrechnungssystem für `mand` und `cust`. Fünfte Datenbank `u14bc1w8_v08_subscriptiondb`, Laravel-Connection `subscriptiondb`, Base-Model `App\Models\SubscriptionDb\SubscriptionDbModel` (minimal, analog den vier bestehenden Base-Models: nur `$connection`).

**Fünf Tabellen:**
- `subscriber` — Vertragspartner, überdauert gelöschte Accounts (Aufbewahrungspflicht)
- `plan` — versionierte Tarife (Preisänderung = neue Zeile, nie Update)
- `subscription` — Vertrag je Nutzer; ein Subscriber kann mehrere Verträge für mehrere Nutzer zahlen
- `ledger_entry` — unveränderliches Buchungsjournal, fünf Buchungsarten `FO`/`GG`/`ZE`/`ZG`/`ZA`, zwei getrennte Betragsspalten (`money_amount` fürs Geldkonto, `amount` für den Vertragssaldo)
- `invoice` — Rechnung als Momentaufnahme je Vertrag, kein eigener Forderungsträger (der bleibt beim Ledger)

**Kein Eingriff in bestehende Tabellen.** Verknüpfung zu `userdb.mand_user`/`userdb.cust_user` ausschließlich logisch über `user_type` + `user_id` — analog dem etablierten Muster bei `passkey`, `passkey_dismissed` und `trusted_device`.

**Abweichung vom Projektstandard:** Innerhalb `subscriptiondb` werden bewusst FK-Constraints gesetzt (alle `ON DELETE RESTRICT`) — sie dokumentieren die Verwendung der Datenbank und machen das Modell ohne zusätzliche Erklärung lesbar. Sonst gibt es im Projekt nur einen einzigen echten FK (siehe Abschnitt 4, Callout). Details und Begründung: `docs/Konzept_Abrechnungssystem.md` Abschnitt 5.4.

**Anwendungslogik bewusst NICHT implementiert.** Abschnitt 4 des Konzepts beschreibt die Buchhaltungsabläufe (Umlage von Zahlungseingängen, FIFO-Ausgleich, Erstattung) als Erweiterungsoption für eine mögliche spätere kommerzielle Nutzung — dafür fehlt aktuell ein Buchhaltungsprogramm.

**Umsetzungsstand (29.08.):** DDL für alle fünf Tabellen ausgeführt (inkl. FK-Constraints). Illustrierender Demo-Datensatz eingespielt — Subscriber 30118 mit drei Verträgen, entspricht exakt dem Buchungsbeispiel aus Konzept-Abschnitt 4.2. Trigger für die Unveränderbarkeit des Journals (`BEFORE UPDATE`/`BEFORE DELETE` auf `ledger_entry`) **noch nicht** gesetzt. Noch nicht umgesetzt: Tabellen-Models, Controller, Views, jegliche Buchhaltungslogik.

**Nebenbei behoben:** Die bereits ausgeführte DDL hatte `entry_type` fälschlich mit `'LS'` statt `'ZA'` (Zahlungsausgang) angelegt — ein Bug, kein bewusster Namenswechsel gegenüber dem Konzept. Korrigiert per `ALTER TABLE`+`UPDATE`, siehe Inkonsistenzen.md.

**Details:** vollständiges Konzept in `docs/Konzept_Abrechnungssystem.md`.

**Tag:** subscriptiondb_vorbereitet_ok (Commit 47843f2).

---

## 10o. Passkey-Fixes, Mobile-UI-Überarbeitung, Samsung-Blockade, Galeristen-Löschschutz — 31.08.–04.09.2026

**GPM-Doppelregistrierung verhindert** (Tag `passkey_gpm_doppelregistrierung_fix_ok`, Commit `24afe41`): Beim Registrieren eines neuen Passkeys wurde bisher nicht geprüft, ob für dasselbe Gerät/denselben Authenticator (insbesondere den Google Password Manager, GPM) bereits ein Credential existiert — dadurch konnten Doppelregistrierungen entstehen. Fix: AAGUID-Prüfung sowie Übergabe der bereits vorhandenen Credential-IDs als `excludeCredentials` an den Registrierungs-Request verhindern jetzt, dass derselbe Passkey ein zweites Mal registriert wird.

**Mobile Kartenansicht für „Meine Passkeys"** (Tag `passkey_test_mobile_ui_ok`): Analog zum bereits etablierten Muster (System-User-/Galeristen-Liste, siehe unten) wurde die Passkey-Übersicht (`customer/passkey/index.blade.php`, `mandant/passkey/index.blade.php`) auf schmalen Bildschirmen umgebaut — die Desktop-Tabelle ist nur noch ab `md:` sichtbar, darunter erscheint ein Detailbereich mit dem ausgewählten Passkey plus einer senkrechten Tab-Liste zum Umschalten.

**iOS: Dismiss-Logik und Namensvorschlag korrigiert:** `LoginSessionBuilder` prüfte beim Ermitteln, ob der „Passkey einrichten?"-Prompt erneut angezeigt werden soll, bisher den vollen `ua_hash` (inkl. Browser-Anteil) — auf iOS teilen sich Safari, Chrome und Firefox jedoch denselben iCloud-Schlüsselbund, sodass „Nie wieder fragen" faktisch pro Browser statt pro Gerät galt. Fix: bei `os === 'ios'` wird `ua_hash` beim Abgleich gegen `passkey_dismissed` jetzt ignoriert (nur `user_type`+`user_id`+`os` geprüft). Zusätzlich zeigt der Passkey-Namensvorschlag bei iOS jetzt nur noch „iOS" statt „iOS – Safari"/„iOS – Chrome" etc., da die Browser-Angabe dort ohnehin irreführend war.

**Samsung Internet Browser wird komplett blockiert** (Tag `samsung_browser_blocked_ok`, Commit `2e1caf5`): Neue Middleware erkennt den Samsung-Internet-User-Agent und leitet auf eine eigene Hinweisseite um, statt den Login zu erlauben.

**Mobile Kartenansicht für System-User- und Galeristen-Liste:** `system/users/index.blade.php` und `system/mandanten/index.blade.php` erhielten dasselbe responsive Muster wie oben beschrieben (Desktop-Tabelle nur ab `md:`, darunter Detailbereich + Tab-Liste). Die Galeristen-Liste wurde zusätzlich 30 % breiter (`max-w-6xl` statt `max-w-4xl`), da die zusätzliche Aktions-Spalte auf der bisherigen Breite zu eng wurde.

**Eigenes Lösch-Bestätigungsmodal für Galeristen:** Der native `confirm()`-Dialog beim Löschen eines Galeristen-Kontos wurde durch ein eigenes Alpine-Modal ersetzt (Name des Galeristen fett hervorgehoben, Warntext aus der editierbaren UI-Text-Datei `storage/app/private/ui-texte/syst/s_mand_delete_warnung.md`) — notwendig, da hier ein mehrzeiliger Fließtext mit Aufzählung benötigt wird, den ein natives `confirm()` nicht darstellen kann.

**Zweistufiger Löschschutz für Galeristen-Konten** (Tag `mand_deaktivierung_loeschschutz_ok`, Commit `929bbf3`): Bisher konnte ein Galeristen-Konto von `syst` direkt und endgültig gelöscht werden. Neu: `syst` kann ein Konto stattdessen zunächst **deaktivieren** (neue Route+Methode `SystemMandantController::toggleActive()`) — der Galerist erhält eine E-Mail-Benachrichtigung (`MandDeactivatedMail`), der Deaktivierungs-Zeitpunkt wird in neuem Feld `mand_user.mand_deactivated_at` (datetime, nullable) gespeichert. Endgültiges Löschen ist erst nach Ablauf einer editierbaren Karenzzeit möglich (neue Config `config/mand_deactivation.php`, Kontakt-E-Mail + Karenzzeit aus `.env`, `MAND_DELETE_GRACE_DAYS`, Standard 7 Tage) und ausschließlich noch über die Bearbeiten-Seite erreichbar — der Löschen-Button wurde aus der Liste entfernt. Reaktivierung ist jederzeit möglich (`MandActivatedMail`).

**Passkey-Gesamttest weiterhin nicht abschließend spezifiziert:** Die obigen Punkte stellen erheblichen Fortschritt dar, decken aber nicht nachweislich den vollständigen in Abschnitt 9/16a-0 beschriebenen Testumfang (alle Rollen × Geräte × Browser × Grenzfälle) ab.

**Tags:** siehe Abschnitt 13.
...
| 27 | **Logout-Button-Komponente:** Trusted-Device-Bestätigungsdialog hatte nur „Abmelden mit Löschen" / „Zurück" (schließt nur den Dialog). Eine besprochene Umbenennung zu „Verlassen ohne Löschen" (Tab schließen statt Dialog schließen) wurde zunächst NICHT umgesetzt (Nutzer hatte abgebrochen) | **BEHOBEN 04.08.2026** (Tag `logout_dialog_close_window_ok`, Commit `859516d`) — Button heißt jetzt „Fenster schließen", ruft zusätzlich `window.close()` auf; Dialog-Fallback (`showConfirm = false`) bleibt erhalten. Details Abschnitt 10i |
...
- **03.08.:** `trusted_device_cust_nofactor_fix_ok` (Commit `ddf5e55`) — cust-Trusted-Device-Cookie im Nicht-2FA-Login-Pfad nachgezogen (`CustLoginController::issueTrustedDeviceIfRequested()`, analog `MandantLoginController`). Details Abschnitt 10g.
- **04.08.:** `trusted_device_passkey_ok` (Commit `268c2b7`) — Trusted-Device-Cookie auch im Passkey-Login-Pfad wirksam (cust+mand), JS liest Checkbox-Zustand aus dem DOM, `issueTrustedDeviceIfRequested()`-Typehint auf `RedirectResponse|JsonResponse` gelockert. Details Abschnitt 10h. Danach `logout_dialog_close_window_ok` (Commit `859516d`) — Logout-Dialog-Button „Zurück" zu „Fenster schließen", `window.close()`-Versuch mit Dialog-Fallback, löst Inkonsistenz #27. Details Abschnitt 10i.
- **26.08.:** `pflichtfelder_konfiguration_ok` (Commit `eef2eed`) — konfigurierbare Pflichtfelder für mand/cust (Telefon/Straße+Hausnr./PLZ+Ort/Firma) über neue Datei `storage/app/private/pflichtfelder.txt` + Helper `istPflichtfeld()`; begleitende DB-Migration `userdb.*_tel`/`*_street+nr`/`*_postcode(_city|+city)`/`*_company` auf `NULL DEFAULT NULL`; neue mand-Detailseite je Mitglied (`mandant.kunden.show`) + umgebaute Mitgliederliste; syst sieht jetzt zusätzlich mand-Adresse; Pflicht-Sternchen-Bugfix in `mandant/konto.blade.php`. Details Abschnitt 10j.
- **26.08.:** `pflichtfelder_erzwingung_ok` (Commit `16e81f3`) — neue Middleware `CheckPflichtfelder` erzwingt fehlende, neu zur Pflicht gewordene Felder nach Login (Redirect zur Konto-Seite mit Amber-Hinweisbox, 60s-Cache); grauer „(optional)"-Hinweis bei Straße/PLZ+Ort ergänzt. Details Abschnitt 10k.
- **26.08.:** `registration_policy_version_fix_ok` (Commit `83c738e`) — Bugfix: `ds_version`/`upload_terms_version` bei Registrierung lesen jetzt `PolicyVersion::get()` statt statischem Config-Wert; verhinderte fälschliches „Policy geändert"-Popup direkt nach Registrierung. Details Abschnitt 10l.
- **27.08.:** `ui_texte_ausgelagert_ok` (Commit `2dbeb37`) — 28 UI-/E-Mail-Textstellen nach `storage/app/private/ui-texte/{all,cust,mand,syst}/*.md` ausgelagert (neuer Helper `uiText()`, `renderMarkdownVariant()` um `$vars`-Platzhalter erweitert); 2FA-Code-Gültigkeitsdauer über neue `config/twofa.php` zentralisiert (vorher an zwei Stellen hartcodiert). Zusätzlich `.env`-only (nicht versioniert): Mail-Konfiguration auf echtes Postfach (`smtps`, Alfahosting) korrigiert, `BACKSTAGE_PATH` erstmals explizit gesetzt. Details Abschnitt 10m.
- **31.08.–04.09. (neu, aktueller Stand):** `subscriptiondb_vorbereitet_ok` (Commit `47843f2`) — fünfte Datenbank `subscriptiondb` vorbereitet (5 Tabellen, Grundlage für ein späteres Abrechnungssystem, Anwendungslogik noch nicht implementiert). `passkey_gpm_doppelregistrierung_fix_ok` (Commit `24afe41`) — AAGUID-Prüfung + `excludeCredentials` verhindert Doppelregistrierung desselben Passkeys bei Google Password Manager. `passkey_test_mobile_ui_ok` — Mobile Kartenansicht für „Meine Passkeys" (Desktop-Tabelle ab `md:`, Detailbereich + Tab-Liste); kein Commit-Hash sicher zuordenbar. `samsung_browser_blocked_ok` (Commit `2e1caf5`) — Samsung Internet Browser wird komplett blockiert (eigene Hinweisseite statt Login). `mand_deaktivierung_loeschschutz_ok` (Commit `929bbf3`) — zweistufiger Löschschutz für Galeristen-Konten: Deaktivieren mit E-Mail-Benachrichtigung + Karenzzeit (`mand_user.mand_deactivated_at`, `config/mand_deactivation.php`), endgültiges Löschen erst danach über die Bearbeiten-Seite. Details Abschnitt 10o.
- `.gitignore`: `.env`, `/vendor/`, `/node_modules/`, `/storage/logs/`, `fotosite_DDL_*.sql`
- `storage/app/.gitignore`: `*` (pauschal) — betrifft u.a. `storage/app/private/ui-texte/**/*.md` (Abschnitt 10m), `pflichtfelder.txt`, `honeypot_paths.txt`: diese Dateien existieren nur auf dem Server bzw. lokal, werden per WinSCP synchron gehalten, nicht per Git.
...
## 16. Offene Aufgaben & Anschlusspunkte (Stand 04.09.2026)
...
> **Hinweis 04.09.:** Seit dem 27.08.-Stand wurde erheblicher Fortschritt erzielt — GPM-Doppelregistrierung-Fix, Mobile-UI für die Passkey-Liste, iOS-Dismiss-/Namensvorschlag-Fixes, Samsung-Internet-Blockade sowie der zweistufige Löschschutz für Galeristen-Konten (Details Abschnitt 10o). Am Passkey-**Testbedarf** selbst hat sich dadurch nichts geändert: die obigen Punkte decken nicht nachweislich den vollständigen Testumfang (alle Rollen × Geräte × Browser × Grenzfälle) ab, der Rest-Umfang ist noch nicht abschließend spezifiziert — der Gesamttest bleibt unverändert oberste Priorität. Zusätzlich vor Phase 7 zu erledigen: syst-Passwort-Policy committen (aktuell uncommitted; Honeypot-Infrastruktur ist bereits committed, Tag `honeypot_login_attacks_log_ok`, siehe Abschnitt 13), `HONEYPOT_LOCKOUT_MINUTES` und `LOG_STACK=daily` auf der Server-`.env` nachtragen (siehe Abschnitt 8a).
...
5. ~~**Logout-Button-Dialog:** „Zurück"-Button ggf. zu „Verlassen ohne Löschen" umbenennen~~ **Erledigt 04.08.2026** — Button heißt jetzt „Fenster schließen", zusätzlicher `window.close()`-Versuch (Tag `logout_dialog_close_window_ok`, siehe Abschnitt 10i, Inkonsistenz #27).
