...
*Stand: 4. September 2026  |  Letzter Git-Tag: mand_deaktivierung_loeschschutz_ok*
...
**⚠  NEU 28.–29.08.: Fünfte Datenbank subscriptiondb vorbereitet (Grundlage für ein späteres Abrechnungssystem für mand+cust, noch keine Anwendungslogik) — DDL für 5 Tabellen ausgeführt (subscriber, plan, subscription, ledger_entry, invoice), Connection subscriptiondb + Base-Model SubscriptionDbModel angelegt, Demo-Datensatz eingespielt (Subscriber 30118, 3 Verträge). ACHTUNG: DB-Passwort aktuell schwach (Testphase) — vor Produktivnutzung ersetzen. Trigger für Journal-Unveränderbarkeit noch NICHT gesetzt. Tag subscriptiondb_vorbereitet_ok (Commit 47843f2). Details Abschnitt 5.2n, vollständiges Konzept: docs/Konzept_Abrechnungssystem.md.**
...
| Letzter Git-Tag | mand_deaktivierung_loeschschutz_ok (04.09.2026). Meilenstein: user_management_complete_ok (20.06.) |
...
**🎯  NAECHSTER SCHRITT (oberste Prioritaet): Gruendlicher Gesamttest der Passkey-Funktionalität. Phase 6 ist technisch implementiert, aber noch nicht systematisch getestet. Der ausstehende Test umfasst Registrierung, Login, Umbenennen, Loeschen, Prompt-/Dismiss-Logik, jeweils fuer mand UND cust, ueber Windows/Android/iOS und die relevanten Browser hinweg - AUSDRUECKLICH KEIN reiner iOS-Test. Details Abschnitt 7. Seit dem letzten Stand (27.08.) wurde erheblicher Fortschritt erzielt (GPM-Doppelregistrierung-Fix, Mobile-UI fuer die Passkey-Liste, iOS-Dismiss-/Namensvorschlag-Fixes, Samsung-Blockade - Details Abschnitt 5.2o), der verbleibende Rest-Umfang des Gesamttests ist jedoch noch nicht abschliessend spezifiziert.**
...
## 5.2o Passkey-Fixes, Mobile-UI, Samsung-Blockade, Galeristen-Löschschutz 31.08.–04.09.2026

✓  GPM-Doppelregistrierung verhindert (passkey_gpm_doppelregistrierung_fix_ok, Commit 24afe41): AAGUID-Pruefung + excludeCredentials verhindert, dass derselbe Passkey beim Google Password Manager doppelt registriert wird

✓  Mobile Kartenansicht fuer "Meine Passkeys" (passkey_test_mobile_ui_ok): Desktop-Tabelle nur noch ab md: sichtbar, darunter Detailbereich + Tab-Liste fuer schmale Bildschirme

✓  iOS: Dismiss-Logik ignoriert browserspezifischen ua_hash — Safari/Chrome/Firefox teilen sich auf iOS denselben iCloud-Schluesselbund, "Nie wieder fragen" wird jetzt browseruebergreifend respektiert statt pro Browser separat abgefragt. Zusaetzlich: Passkey-Namensvorschlag zeigt bei iOS nur noch "iOS" ohne Browser-Suffix

✓  Samsung Internet Browser wird komplett blockiert (samsung_browser_blocked_ok, Commit 2e1caf5): eigene Hinweisseite statt Login-Moeglichkeit

✓  Mobile Kartenansicht fuer System-User- und Galeristen-Liste: Desktop-Tabelle ab md:, darunter Detailbereich + Tab-Liste; Galeristen-Liste zusaetzlich 30% breiter (max-w-6xl statt max-w-4xl)

✓  Eigenes Loesch-Bestaetigungsmodal fuer Galeristen: ersetzt natives confirm(), Name fett hervorgehoben, Warntext aus editierbarer UI-Text-Datei (storage/app/private/ui-texte/syst/s_mand_delete_warnung.md)

✓  NEU — Zweistufiger Loeschschutz fuer Galeristen-Konten (mand_deaktivierung_loeschschutz_ok, Commit 929bbf3): Syst kann ein Galeristen-Konto deaktivieren statt sofort zu loeschen — Galerist erhaelt eine E-Mail-Benachrichtigung, Zeitpunkt wird in mand_user.mand_deactivated_at gespeichert. Endgueltiges Loeschen ist erst nach Ablauf einer editierbaren Karenzzeit moeglich (.env-Wert MAND_DELETE_GRACE_DAYS, Standard 7 Tage) und nur noch ueber die Bearbeiten-Seite (Loeschen-Aktion wurde aus der Liste entfernt). Neues DB-Feld mand_user.mand_deactivated_at (datetime, nullable), neue Config config/mand_deactivation.php, neue Route+Methode SystemMandantController::toggleActive(), neue Mailables MandDeactivatedMail/MandActivatedMail

⚠  Passkey-Gesamttest weiterhin nicht abschliessend spezifiziert: die obigen Punkte stellen erheblichen Fortschritt dar, decken aber nicht nachweislich den vollstaendigen in Abschnitt 7 beschriebenen Testumfang (alle Rollen x Geraete x Browser x Grenzfaelle) ab. Siehe NAECHSTER SCHRITT oben und Abschnitt 7
...
Vollstaendiges Konzept: docs/Konzept_Abrechnungssystem.md. Tag subscriptiondb_vorbereitet_ok (Commit 47843f2). Details PROJECT_CONTEXT.md Abschnitt 10n.
...
**⚠  NEU 31.08.–04.09.: Erheblicher Fortschritt seit dem letzten Stand (27.08.), Rest-Umfang des Gesamttests noch nicht abschliessend spezifiziert** — GPM-Doppelregistrierung-Fix (passkey_gpm_doppelregistrierung_fix_ok): AAGUID-Pruefung + excludeCredentials verhindert Doppelregistrierung desselben Passkeys beim Google Password Manager. Mobile Kartenansicht fuer "Meine Passkeys" (passkey_test_mobile_ui_ok): Desktop-Tabelle ab md:, darunter Detailbereich + Tab-Liste. iOS-Dismiss-Logik ignoriert jetzt browserspezifischen ua_hash (Safari/Chrome/Firefox teilen sich auf iOS den iCloud-Schluesselbund), Passkey-Namensvorschlag zeigt bei iOS nur noch "iOS" ohne Browser-Suffix. Samsung Internet Browser wird komplett blockiert (samsung_browser_blocked_ok, eigene Hinweisseite statt Login). Details Abschnitt 5.2o.**
